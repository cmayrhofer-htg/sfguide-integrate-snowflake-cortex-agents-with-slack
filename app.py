...
PAT = os.getenv("PAT")
print(
    f"SLACK_APP_TOKEN: {SLACK_APP_TOKEN[:10]}*** and "
    f"SLACK_BOT_TOKEN: {SLACK_BOT_TOKEN[:10]}***."
)
...
def handle_message_event(event, say, client, body):
    ...
    try:
        ...
        if not user_message:
            say(
                "👋 Hi! Ask me any question about your data and I'll help you analyze it using Snowflake Cortex."
            )
            return
    ...
    try:
        ...
        if action_value == "show":
            ...
            if combined_steps:
                ...
                max_content_length = 2950 - len(
                    header
                )  # Leave room for header and safety margin
            ...
            try:
                ...
                if sql_queries:
                    ...
                    for i, sql_query in enumerate(sql_queries, 1):
                        ...
                        is_verified = verified_query_used and (
                            i == 1 or num_queries == 1
                        )
                        ...
                        if is_verified:
                            query_header += (
                                " :verified: Answer accuracy verified by agent owner"
                            )
                        ...
                        blocks.extend(
                            [
                                {
                                    "type": "section",
                                    "text": {"type": "mrkdwn", "text": query_header},
                                },
                                {
                                    "type": "section",
                                    "text": {
            ...
            if timeline:
                ...
                step_count = (len(steps) if steps else 0) + (
                    len(thinking_steps) if thinking_steps else 0
                )

            # Check what additional info is available
            additional_info = []
            try:
                has_verification = getattr(
                    CORTEX_APP, "verification_info", {}
                ) or getattr(CORTEX_APP, "verified_query_used", False)
                sql_queries = getattr(CORTEX_APP, "sql_queries", [])
                ...
                if has_verification and sql_queries:
                    ...
                    additional_info.append(
                        f":verified: answer accuracy verified by agent owner for {query_count} SQL {'query' if query_count == 1 else 'queries'}"
                    )
                    ...
                    query_count = len(sql_queries)
                    additional_info.append(
                        f"{query_count} SQL {'query' if query_count == 1 else 'queries'}"
                    )
            except:
                pass
        ...
        app.client.chat_update(
            channel=channel_id, ts=message_ts, text="🤔 Planning details", blocks=blocks
        )
...
def smart_truncate(text, max_length=300, suffix="..."):
    ...
    return (
        truncated.strip() + suffix
        if truncated.strip()
        else text[: max_length - len(suffix)] + suffix
    )
    ...
    try:
        ...
        if content.get("suggestions"):
            ...
            formatted_suggestions = [
                format_text_for_slack(suggestion)
                for suggestion in content["suggestions"][:3]
            ]
            suggestions_text = "\n".join(
                f"• {suggestion}" for suggestion in formatted_suggestions
            )
            say(
                text="💡 Suggestions",
                blocks=[
    ...
    try:
        ...
        try:
            conn = snowflake.connector.connect(
                user=USER, password=PAT, account=account, warehouse=WAREHOUSE, role=ROLE
            )
            ...
            print(
                f"   ✅ PAT authentication successful! Snowflake version: {result[0]}"
            )
            return conn
...
def init():
    ...
    cortex_app = cortex_chat.CortexChat(AGENT_ENDPOINT, PAT)
